    ...
    <div class="box-header">
      ...
      <div class="container-fluid">
        <div class="row">
          ...
          <div class="col-md-6">
            <h1 class=""><span class="glyphicon glyphicon-categories"> </span>  {{$category->name}} category <small>{{$category->actions->count()}} {{($category->actions->count() <= 1)? ' Action' : ' Actions'}}</small></h1>
          </div>
          <br>
            <div class="col-md-2">
            ...
            <div class="col-md-2">
              <a class="btn btn-default btn-block" href="{{route('action_categories.index')}}" ><span class="glyphicon glyphicon-eye-open"> </span> All</a>
            </div>
      ...
      <table id="example1" class="table table-hover">
        ...
        <tbody>
          @foreach ($category->actions as $action)
            <tr>
              <td>{{ $action->id}}</td>
              <td><a href="{{route('actions.show', $action->id)}}">{{ $action->title }}</a></td>
              <td><a href="{{route('action_categories.show', $action -> commune -> id)}}">{{ $action -> commune -> name}}</a></td>
              <td>
                <a href="{{route('actions.show', $action->id)}}" class="btn btn-default btn-xs" ><span class="glyphicon glyphicon-eye-open"></span> View</a>
              </td>
            </tr>
          @endforeach
        </tbody>
          ...
          <div class="container-fluid">
            <div class="row">
              <div class="col-md-10 col-md-offset-1">
                ...
                <form name="sentMessage" id="contactForm" action="{{route('action_categories.update', $category->id)}}" method="POST" data-parsley-validate>
        ...
        <div class="modal-body">
          ...
          <div class="container-fluid">
            <div class="row">
              ...
              <div class="col-xs-4">
                <form name="sentMessage" id="contactForm" action="{{route('action_categories.destroy', $category->id)}}" method="post" data-parsley-validate>
                  <input type="hidden" name="_token" value="{{csrf_token()}}">
                  {{method_field("DELETE")}}
